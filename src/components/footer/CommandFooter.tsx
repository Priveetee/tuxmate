...
import { Check, Copy, Download, X, ChevronUp } from "lucide-react";
...
export function CommandFooter({
  ...
  const handleCopy = async () => {
    ...
    const distroName =
      distros.find((d) => d.id === selectedDistro)?.name || selectedDistro;
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex items-center gap-2 sm:gap-3">
            <span
              className="text-xs sm:text-sm text-[var(--text-muted)] whitespace-nowrap tabular-nums hidden sm:block font-medium min-w-[20px]"
              style={{ transition: "color 0.5s" }}
            >
              {selectedCount}
            </span>
            ...
            >
              <div className="flex items-start gap-3 px-4 pt-3 pb-1">
                <div className="flex-1 min-w-0 overflow-x-auto command-scroll">
                  <code
                    className={`whitespace-nowrap ${selectedCount > 0 ? "text-[var(--text-secondary)]" : "text-[var(--text-muted)]"}`}
                    style={{ transition: "color 0.5s" }}
                  >
                    {command}
                  </code>
            ...
            >
              <button
                ...
                className={`h-11 w-11 sm:w-auto sm:px-4 rounded-xl text-sm flex items-center justify-center gap-2 transition-all duration-200 outline-none ${
                  selectedCount > 0
              ...
              {showDownloadTooltip && (
                <div
                  className="absolute bottom-full right-0 mb-2 px-3 py-2 bg-[var(--bg-tertiary)] text-[var(--text-secondary)] text-sm rounded-lg shadow-xl border border-[var(--border-secondary)] whitespace-nowrap"
                  style={{ animation: "tooltipSlideUp 0.3s ease-out forwards" }}
                >
                  Download install script
                  <div
                    className="absolute right-4 translate-x-1/2 w-2.5 h-2.5 bg-[var(--bg-tertiary)] border-r border-b border-[var(--border-secondary)] rotate-45"
                    style={{ bottom: "-6px" }}
                  />
                </div>
              )}
            ...
            <div className="relative flex items-center">
              <button
                onClick={handleCopy}
                disabled={selectedCount === 0}
                className={`h-11 px-5 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all duration-200 outline-none ${
                  selectedCount > 0
                    ? copied
              ...
              {showCopyTooltip && (
                <div
                  className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-4 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg shadow-xl whitespace-nowrap"
                  style={{ animation: "tooltipSlideUp 0.3s ease-out forwards" }}
                >
                  Paste this in your terminal!
                  <div
                    className="absolute left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-emerald-600 rotate-45"
                    style={{ bottom: "-5px" }}
                  />
                </div>
              )}
      ...
      {drawerOpen && (
        <>
          ...
          <div
            ...
            style={{
              animation: drawerClosing
                ? "slideDown 0.25s ease-in forwards"
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)]">
                    {selectedCount} app{selectedCount !== 1 ? "s" : ""} • Press
                    Esc to close
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between px-4 py-2 bg-[#252525] border-b border-[var(--border-primary)]">
                  ...
                  <div className="hidden md:flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`h-7 px-3 flex items-center gap-1.5 rounded-md text-xs font-medium transition-all ${
                        copied
                          ? "bg-emerald-600 text-white"
            ...
            <div className="md:hidden flex flex-col items-stretch gap-3 px-4 py-4 border-t border-[var(--border-primary)]">
              ...
              <button
                ...
                className={`flex-1 h-14 flex items-center justify-center gap-2 rounded-xl font-medium text-base transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  copied
